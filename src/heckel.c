...
typedef struct
{
    ...
    int other;			/* line number of equivalent line in other file,
				   or 0 if not matched */
} array_entry;
 ...
 *   Here we have a list of line numbers in the 2 buffers. We keep a count
 *   of the 2 lists.
...
typedef struct hash_node {
	...
	int num_desired;
	int new[MAX_OTHER];
	int key;	/* This is the hash. an int representing the
			   string/line-text. */
} hash_type;
...
/* mmc:
 * lookup in Hash. If not present, add (at the head of the same-hash chain),
 * a hash_node w/ 2 more bytes:	 */
/* INTPROTO */
hash_type*
...
/* Walks master array.	Through the hashtable, find the points-of-equality, and
 * `attach' them.  (link original lines in files.)
 */
/* INTPROTO */
void
attach_corresponding(int num_lines, array_entry* current, array_entry* desired, hash_table *hash)
{
	...
	if ((value->num_current == value->num_desired)
		...
		for (i = 0; (i < value->num_desired) ; i++) {
		    /* fixme: */
		    if ((desired[value->new[i]].other == -1) &&
			(current[value->old[i]].other == -1))
			/* mmc: at the beginning .other = 0! */
			{
				...
				{/* link previous array entry */
				    ...
				    if ((! desired[value->new[i]-2].other) &&
					(! current[value->old[i]-2].other))
					{
...
/* So, we have 2 equal-size buffers, r->screen (or ->swap, or ->snapshot) and
 * r->drawn_text.  We want to run heckel on them, and get: scolling-index and a
 * bitmap of changed rows (vs. simply scrolled rows) */
 /* len[] */
/* INTPROTO */
void
