...
/* fill explicitely, instead of implicity fill by X server, on Expose */
void
my_fill_area(Display* dpy, Drawable d, GC gc, int x, int y, int width, int height)
{
#if 0                           /* mmc_debug */
...
rxvt_scr_expose(rxvt_t *r, int x, int y, int width, int height, Bool refresh)
{
    ...
    my_fill_area(r->Xdisplay, r->TermWin.vt, r->TermWin.background_gc, /* r->h->colorfgbg */
                 0,0,
                 /* mmc: unfotrunately the text drawing start offset  */
...
rxvt_scr_reverse_selection(rxvt_t *r)
{
    ...
    /* r->selection contains the necessary info on what is selected.
       r->screen.rend contains the (color) attributes for feedback (due to selection)
     */
    /* r->h->current_screen ?? */
    /* .op is the state of what the user is doing w/ selection. 0 if no selection is made.*/
    if (r->selection.op && r->h->current_screen == r->selection.screen) {
...
rxvt_selection_paste(rxvt_t *r, Window win, Atom prop, Bool delete_prop)
{
	...
	if (XmbTextPropertyToTextList(r->Xdisplay, &ct, &cl,
            ...
            rxvt_PasteIt(r, (unsigned char*)cl[0], STRLEN(cl[0])); /* fixme! */
	    XFreeStringList(cl);
	} else
#endif
...
#ifndef NO_FRILLS
/* mmc: This removes the "empty" (space/tab only) lines at the end of the selection.
 * And trailing space/tabs on the last non-empty line. */
/* INTPROTO */
void
rxvt_selection_remove_trailing_spaces(rxvt_t *r)
{
	...
	if (end_col >= 0
	    || r->screen.tlen[end_row - 1 + r->TermWin.saveLines] != -1) { /* wrapped  */
