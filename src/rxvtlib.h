...
/*
 * section 1 generated by GNU autoconf for x86_64-unknown-linux-gnu
...
typedef struct {
    ...
    GC              background_gc; /* GC to make AreaFills: Since i avoid
                                   * ClearArea, I explicitely fill with the
                                   * background color.  But to do that, I
                                   * need a GC with the hypothetical window
                                   * background color set as _foreground_ of
                                   * this suitable GC. */
 ...
 */
    ...
    text_t        **buf_text;   /* mmc: is it for temporary pointers to cycled rows? when scrolling */
    rend_t        **buf_rend;
    char           *tabs;	/* per location: 1 == tab-stop               */
    screen_t        screen;
    screen_t        swap;
    screen_t        snapshot;   /* mmc: when needed, we detach output from working (being written to)
                                 * buffer(s): So, we display (into X server) from snapshot, but
                                 * the subprocess works on screen/swap. */
